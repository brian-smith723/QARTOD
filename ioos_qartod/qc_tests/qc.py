...
def st_time_seires_gap(arr, N, start=0, end=0):
    ...
    # a specific time. It is necessary to know the interval in which
    # observations are taken so that the number of missing points can be
    # computed

    #  If N is 'time', not 'integer'. E.g, if obs are taken every hour
    # and data will fail if 6 hours is missed, N = 6 * 3600000000000.
    # Operator needs to define the  value of time is, i.e. hours, seconds, days
    # time is diffed in nanoseconds.
...
def lt_time_series_stuck_sensor(arr, EPS):
    ...
    # A moving average is used here, and the three most recent observations,
    # ie P0n are flagged to suspect
...
def st_time_series_acceleration(arr, M, N):
    ...
    # need further information before this test can be finished.
